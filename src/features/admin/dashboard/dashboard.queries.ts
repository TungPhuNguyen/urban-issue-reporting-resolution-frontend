...
export const adminDashboardKeys = {
  ...
  detail: (range: DashboardDateRange) =>
    [...adminDashboardKeys.all, range.fromDate, range.toDate] as const,
}

export function useAdminDashboard(range: DashboardDateRange) {
  return useQuery({
    queryKey: adminDashboardKeys.detail(range),
    queryFn: () => adminDashboardApi.getDashboard(range),
    staleTime: 30_000,
  })
}
