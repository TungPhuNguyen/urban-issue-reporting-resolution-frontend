...
export default function AreasPage() {
  ...
  const [form, setForm] =
    useState<AreaFormState>(EMPTY_FORM)
  ...
  useEffect(() => {
    if (
      areasQuery.data &&
      pageNumber > 1 &&
      areasQuery.data.items.length === 0
    ) {
      setPageNumber(
        Math.max(
          1,
  ...
  function validateForm() {
    ...
    const parentAreaId =
      form.parentAreaId
        ? Number(form.parentAreaId)
        : null

    if (
      parentAreaId !== null &&
      (!Number.isInteger(parentAreaId) ||
        parentAreaId <= 0)
    ) {
      return {
        error:
          'Khu vực cha không hợp lệ.',
      }
    }

    if (
      editingArea &&
      parentAreaId === editingArea.id
    ) {
      return {
        error:
  ...
  const parentOptions =
    parentAreasQuery.data?.items.filter(
      (area) =>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Quản lý cây khu vực và trạng
            thái hoạt động của từng khu vực.
          </p>
        </div>
      ...
      {showForm && (
        <Card className="border-blue-200 p-6 dark:border-blue-900">
          <h2 className="text-lg font-semibold text-blue-700 dark:text-blue-400">
            {editingArea
              ? `Cập nhật khu vực #${editingArea.id}`
              : 'Tạo khu vực'}
          </h2>
